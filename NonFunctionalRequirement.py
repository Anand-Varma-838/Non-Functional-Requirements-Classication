...
def metrics(name,feature,y_test, prediction_data):
    ...
    a = accuracy_score(y_test,prediction_data)*100
    accuracy.append(a)
    precision.append(p)
    recall.append(r)
    fscore.append(f)
...
def CNN():
    ...
    cnn_model.add(Activation('softmax'))
    cnn_model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    acc_history = cnn_model.fit(wordvec_X, Y1, epochs=3, validation_data=(X_test, y_test))
    predict = cnn_model.predict(X_test)
    prediction_data = np.argmax(predict, axis=1)
    y_test = np.argmax(y_test, axis=1)
...
def fusionCNN():
    ...
    cnn_model.add(Activation('softmax'))
    cnn_model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    acc_history = cnn_model.fit(fusion, Y1, epochs=3, validation_data=(X_test, y_test))
    predict = cnn_model.predict(X_test)
    prediction_data = np.argmax(predict, axis=1)
    y_test = np.argmax(y_test, axis=1)
    metrics("Fusion CNN","Word2Vec", y_test, prediction_data)
...
def predict():
    ...
    for i in range(len(test_vector)):
        ...
        if similarity > 0:
            if classified == 1:
                text.insert(END,str(test[i])+" - Non Functional Requirement Document\n\n")
            else:
                text.insert(END,str(test[i])+" - Functnional Requirement Document\n\n")
...
font1 = ('times', 13, 'bold')
upload = Button(main, text="Upload Pure Document Dataset", command=upload)
...
svmButton = Button(main, text="Run SVM with All Features Embedding", command=runSVM)
svmButton.place(x=700,y=300)
svmButton.config(font=font1)

nbButton = Button(main, text="Run Naive Bayes with All Features Embedding", command=naiveBayes)
nbButton.place(x=700,y=350)
nbButton.config(font=font1)

lrButton = Button(main, text="Run Logistic Regression with All Features Embedding", command=logisticRegression)
lrButton.place(x=700,y=400)
lrButton.config(font=font1)


cnnButton = Button(main, text="Run CNN with All Features Embedding", command=CNN)
cnnButton.place(x=700,y=450)
cnnButton.config(font=font1)

fusionButton = Button(main, text="Run Fusion CNN with All Features Embedding", command=fusionCNN)
